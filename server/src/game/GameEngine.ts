...
import { SOUTIEN_VALUES } from '../../../shared/src/values';
...
const SOUTIEN_TYPES: SoutienType[] = ['INFORMATEUR', 'PICKPOCKET', 'SBIRE', 'CAID'];
...
export class GameEngine {
  ...
  constructor(
    roomId: string,
    player1: { id: PlayerId; name: string },
    player2: { id: PlayerId; name: string }
  ) {
  ...
  getPublicState(forPlayerId: PlayerId): PublicGameState {
  ...
  applyAction(playerId: PlayerId, action: PlayerAction): { success: boolean; error?: string } {
    if (this.state.phase !== 'PLAYING') return { success: false, error: 'La partie est terminée.' };
    if (this.state.currentPlayerId !== playerId) return { success: false, error: "Ce n'est pas ton tour." };
  ...
  private handlePlayCard(playerId: PlayerId, cardId: string): { success: boolean; error?: string } {
    ...
    if (cardIndex === -1) return { success: false, error: 'Carte introuvable dans ta main.' };
  ...
  private handleTakeCards(playerId: PlayerId): { success: boolean; error?: string } {
    const player = this.getPlayer(playerId);
  ...
  private computeScore(player: PlayerState, opponent: PlayerState): FinalScore {
    const soutienScores: SoutienScore[] = [];
    let soutienTotal = 0;

    const playerIndex = (this.state.players as PlayerState[]).indexOf(player) as 0 | 1;
    ...
    for (const type of SOUTIEN_TYPES) {
      ...
      const value = SOUTIEN_VALUES[type];
    ...
    const typesWithCards = SOUTIEN_TYPES.filter(
      (t) => player.collected.some((c) => c.type === 'SOUTIEN' && c.soutienType === t)
    ).length;
  ...
  private tiebreak(p1: PlayerState, p2: PlayerState): PlayerId | null {
    const tiebreakOrder: SoutienType[] = ['CAID', 'SBIRE', 'PICKPOCKET', 'INFORMATEUR'];
    for (const type of tiebreakOrder) {
      ...
      if (c1 > c2) return p1.id;
      if (c2 > c1) return p2.id;
    }
    return null;
  }
