...
io.on('connection', (socket) => {
    ...
    socket.on('join_room', ({ roomId, playerName }) => {
        ...
        if (!room) {
            socket.emit('error', {
                message: 'Partie introuvable. Vérifie le code et que tous les joueurs utilisent la même URL de serveur (ex. tous en local ou tous sur le même hébergement).',
            });
            return;
        }
    ...
    socket.on('reconnect_room', ({ roomId, playerId }) => {
        ...
        if (!result) {
            socket.emit('error', {
                message: 'Impossible de récupérer la partie : code invalide ou la session a été supprimée par le serveur. Retournez à l’accueil et recommencez.',
            });
            return;
        }
        socket.join(code);
